...
def pert_pso(atmlst):
    '''
    Perturbator PSO in molecular basis. 

    Returns the list of the pso perturbator in three dimensions
    must be reshaped like
    np.asarray(h1).reshape(len(atmlst),3,nvir,nocc)

    wich is an array with shape (n,3,nocc,nvir)
    '''
    h1 = []
    orbo = mo[:,:]
    orbv = mo[:,:]
...
#h1 = pert_pso([0,1])
#h1 = np.asarray(h1).reshape(2,3,nprim,nprim)
#propertyimo = h1.reshape(2*3,nprim,nprim)
#print(propertyimo)
###################################################################
