...
class Prop_pol:
    """
    Class with all elements for the calculation of Spin Spin Coupling
    """
...
##########################################################
    ...
    def pert_fc(self,atmlst):
        ''' FC perturbator in molecular orbitals 
        '''
    ...
    def pert_corr(self, pert, multiplicidad):
        '''
        Correction of the perturbation for the HRPA level
        '''
        nocc = self.nocc
        nvir = self.nvir
        k = 0
        ...
        for i in range(nocc):
            for a in range(nvir):
                ...
                if multiplicidad == 1:
                    ...
                    gpvph[a + i * nvir] = -2.0 * pert[ i, s] - (PQA2 + PQB2)
        return gpvph.reshape(nocc,nvir, order='C')
    ...
    def pp_ssc_fc(self, atom1, atom2):
        '''
        method for generate the polarization propagator, as a multiplication
        of a row matrix (h1), the square matris (p) and the row matrix (h2)
        '''
        nvir = self.nvir
        nocc = self.nocc
    ...
    def kernel(self, atom1=None, atom2=None):
        '''
        In this function, we multiplicate the response 
